...
"""
Save the accuracy metrics of the matcher results (query with entity_type="local_authority" provided), for a independent created evaluation
data independent_eval_data.json
run by:
- uv run -m evaluation.independent_data_with_entity_type_eval
...
EXTERNAL_EVALUATION_FILE: Path = BASE_DIR / "independent_eval_data.json"
EXTERNAL_METRICS_FILE: Path = (
    BASE_DIR / "independent_evaluation_entity_type_metrics.json"
)
EXTERNAL_RESULTS_FILE: Path = (
    BASE_DIR / "independent_evaluation_entity_type_results.json"
)
...
def evaluate() -> tuple[Metrics, list[EvaluationResult]]:
    ...
    evaluation_cases: list[EvaluationCase] = load_cases(EXTERNAL_EVALUATION_FILE)
    ...
    for case in evaluation_cases:
        ...
        # Where "expected_type: true" from cases
...
def print_metrics(metrics: Metrics) -> None:
    ...
    print("Independent Data Evaluation with entity_type")
    print("-------------------")
...
if __name__ == "__main__":
    ...
    save_json(EXTERNAL_METRICS_FILE, metrics)
    save_json(EXTERNAL_RESULTS_FILE, results)

    print_metrics(metrics)

    print()

    print(f"Metrics saved to: {EXTERNAL_METRICS_FILE}")
    print(f"Detailed results saved to: {EXTERNAL_RESULTS_FILE}")
